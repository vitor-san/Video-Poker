...
public class Jogador {
	...
	public String toString() {
		...
		//cria a string para guardar cada linha
		java.lang.String[] linha = new java.lang.String[5];
		//inicia todas as strings
		for(int i = 0; i < 5; i++) {
			linha[i] = "";
		}
		
		//cria string que sera usada para separar as partes de cada string
		java.lang.String subString;
		
		//ira guardar o comeco de cada sub string
		int comecoString;
		
		//laco para pegar as partes de cada linha
		for(int i = 0; i < 5; i++) {
			for(int j = 0; j < 5; j++)
			{
				//calcula o comeco da sub string
				comecoString = i*8;
				//retira o pedaco correto da string q deve ser colocada na linha
				subString = Baralho.cartaString(cartas[j]);
				subString = subString.substring(comecoString,comecoString+8);
				//coloca na linha correta
				linha[i] = linha[i] + subString + "\t";
			}
		}
		
		//junta as linhas e a retorna
		return linha[0] + "\n" + linha[1] + "\n" + linha[2] + "\n" + linha[3] + "\n" + linha[4] + "\n" + "\n";
